...
impl CommonConfig {
  fn parsed_date(&self) -> Result<NaiveDate> {
    let tz: Tz = self.parsed_timezone()
...
pub fn parse() -> Result<ParsedOptions> {
  let opts = Options::parse();
  

  match &opts.commands {
    Commands::Location { common, city: _, country: _ } => {
      let date = common.parsed_date()
        .with_context(|| format!("Failed to create date with `{}`", common.date))?;
      let timezone = common.parsed_timezone()
        .with_context(|| format!("Failed to create timezone with `{}`", common.timezone))?;
      let timings = common.parsed_timings()
        .with_context(|| format!("Failed to parse timings with {:?}", common.timings))?;
      let auth = common.parsed_auth()
        .with_context(|| format!("Failed to parse authority with `{}`", common.auth))?;
      // TODO MAKE API CALL WITH CITY AND COUNTRY
      let lat = 43.87982_f64;
      let lng = -78.9421751_f64;
      let school = if common.hanafi { types::School::Hanafi } else { types::School::Shafi };
      return Ok(ParsedOptions::Calculation { date, timezone, lat, lng, timings, auth, school })
    },
    Commands::Coord { common, lat, lng } => {
      ...
      let auth = common.parsed_auth()
        .with_context(|| format!("Failed to parse authority with `{}`", common.auth))?;
      let school = if common.hanafi { types::School::Hanafi } else { types::School::Shafi };
      return Ok(ParsedOptions::Calculation { date, timezone, lat: *lat, lng: *lng, timings, auth, school })
    },
    Commands::Timings => {
...
pub fn display_timings() {
  ...
  writer.write(format!("{}: {}", "Usage".underline(), "salah <location | coords> [OPTIONS] [TIMINGS]...").as_bytes()).unwrap();
  writer.write(b"\n").unwrap();
  writer.write(b"\nThe below can be passed to [TIMINGS]...").unwrap();
  writer.write(b"\n").unwrap();
  writer.write(format!("\n{}:", "Timings".underline()).as_bytes()).unwrap();

  for time in types::Timing::list() {
    writer.write(format!("\n  {:<width$}{:<width$}", time.to_str(), time.desc(), width = 10).as_bytes()).unwrap();
  }
...
pub fn display_authority() {
  ...
  writer.write(format!("{}: {}", "Usage".underline(), "--auth <AUTH>").as_bytes()).unwrap();
  writer.write(b"\n").unwrap();
  writer.write(format!("\n{}:", "Explanation".underline()).as_bytes()).unwrap();
  
  writer.write(b"\nCalculation authorities are used for the calculation of Fajr and Isha.").unwrap();
  ...
  writer.write(b"\nAs this is quite ambiguous, the scholars have differed upon the angle that the sun").unwrap();
  writer.write(b"\nmakes when these two times occur. Each authority has slightly different angles for").unwrap();
  writer.write(b"\nFajr and Isha. Makkah uses a time difference from Maghrib (sunset).").unwrap();
  writer.write(b"\n\nThe below can be used with the --auth <AUTH> option when calculating timings.\n").unwrap();
  writer.write(format!("\n{}:", "Authorities".underline()).as_bytes()).unwrap();

  for auth in types::Authority::list() {
    writer.write(format!("\n  {:<width$}{:<width$}", auth.to_str(), format!("{} - {}", auth.desc(), auth.name()), width = 10).as_bytes()).unwrap();
  }
  writer.write(b"\n").unwrap();
